...
# extracts fitted values from linear model
...
For training misclassification rates, I would choose the model with k = 1 because the training error is the lowest. 
For CV misclassification rates, I would choose the model with k = __ because the misclassification rate is the lowest. 


In practice, I would choose the CV misclassification model because using training error model gives us the minimized error from our training data, but we can't generalize this error rate to our test data. However, with the cv misclassification model, we get an estimate of the test error which is the average of the mean squared errors across the folds. This method is more generalizable, which is why it would be better in practice. 
